...
const F3_WIDE = /(所有|全部|整个|全盘|全局|every|all|system|系统级|.*\*)/i;
function funnel(text) {
    // F1 表层扫描
    const signals = [];
    let weight = 0;
    ...
    let radius = 'none';
    if (stage === 'network-send')
        radius = 'external';
    else if (stage === 'exec-command')
        radius = wide ? 'system' : 'directory';
    else if (stage === 'write-file')
        radius = wide ? 'directory' : 'single-file';
    ...
    const reversible = irreversible ? 'irreversible' : 'reversible';
    // 风险分：F1 权重 + 阶段 + 半径 + 不可逆
    let score = weight;
    if (stage === 'exec-command' || stage === 'network-send')
        score += 0.2;
    if (radius === 'system' || radius === 'external')
        score += 0.2;
    if (irreversible)
        score += 0.1;
...
async function callLocalModel(text, config, signal) {
    const prompt = '你是意图风险分类器。仅输出 JSON：' +
...
function parseModelVerdict(raw) {
...
function compileAction(stage, target, reversible, params) {
    return { stage, target, reversible, params };
}
const KNOWN_STAGES = ['read', 'write-file', 'exec-command', 'network-send', 'message-send', 'query', 'other'];
const SYSTEM_PATH_RE = /(^|[\/\\])(etc|root|Windows|ProgramData|Users[\\\/][^\\\/]+[\\\/]\.ssh|Users[\\\/][^\\\/]+[\\\/]\.config)([\/\\]|$)/i;
function runGates(action, config, modelUsed) {
    ...
    // G2 Schema：动作描述字段齐全
    const g2 = {
        pass: KNOWN_STAGES.includes(action.stage) &&
    ...
    const g4 = { pass: g4Pass, detail: g4Detail };
    return { g1, g2, g3, g4 };
}
function deriveDecision(funnel, gates, config, modelUsed) {
    ...
    const failedGate = ['g1', 'g2', 'g3', 'g4'].find((k) => gates[k].pass === false);
    if (failedGate) {
        return {
            decision: 'BLOCK',
            modelUsed,
...
// ───────────────────────────── 审计 ─────────────────────────────
function audit(ctx, result, config) {
    if (!config.auditLog)
        return;
    const summary = {
