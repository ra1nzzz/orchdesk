...
> **P5 运行门控（与 P1–P4 同源，但已部分解除）**：Electron 现已可安装/运行（BUG-W02 真相为「误用 `node` 跑主进程」，正确启动是 `electron .`）；但**本机无显示器**（`DISPLAY` 空、`NO_XVFB`），GUI 仍无法在此渲染。故 P5 采用「node 下加载真实编译插件 + mock ctx 直驱 `apply()`」验证真实业务逻辑（分类/withhold/补偿/静态门控/授权门控/fail-closed），绕开 GUI；渲染层 UI 经 bridge mock + 乐观本地更新保证可演示。真实 dsh approval/request 应答与沙箱执行验证须转移到可正常运行 Electron/dsh 的机器。
...
7. **dsh 启动机制纠正（防漂移）**：dsh **无 `-b` 参数**；启动单位是 *profile*，`dsh --profile <name>` 从 `$DSH_HOME/profiles/<name>` 加载，其 `package.json` 的 `dsh.profile.bundles` 列表按序叠加 bundle 层（base → desktop → 用户层 → `--patch`）。P1 桌面壳采用「渲染进程持有 UI 状态 + 主进程本地运行时适配器」架构，经 contextBridge 桥接（`load-sessions` / `persist-sessions` / `run-agent-turn`）；dsh 的真实 in-process 集成（`runProfile` + `ctx.agents.followup`）是 **P1-5 模型回合的 design seam**，尚未接通（被 API Key / 显示器门控），届时在 `main.ts:runAgentTurn` 接入。PLAN 中「`dsh -b dsh-desktop`」旧表述已纠正（见 T-P0-2 验收）。
