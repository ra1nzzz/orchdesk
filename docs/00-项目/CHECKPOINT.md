...
| **最新 Commit** | `bc07239` | feat(desktop): PTY 终端 Tab + 只读文件 Tab（ADR-0012，Minke P2-10/11） |
...
| **验证套件** | 786/786 PASS | `npm run verify`（plugins 88 / orchestration 45 / trace-upload 36 / agent-runtime 38 / agent-loop 14 / model-loop 40 / dsh-runtime 31 / credentials 34 / data-dir 47 / data-port 10 / session-fork 29 / memory-promotion 22 / memory-summarize 16 / connector-registry 30 / plugin-market 15 / usage-registry 11 / session-events 16 / ts-loader 13 / browser-tools 43 / **terminal-pty 26 / file-panel 15** / arch-guard 15 / e2e 152）—— 23 套件；另设不进链的真机冒烟 `pnpm run smoke:browser`（11/11，需真 GPU/渲染进程） |
...
*最后更新：2026-09-02（第十五批：PTY 终端 Tab + 只读文件 Tab（ADR-0012，Minke 对照 P2-10/11）｜verify 745→786，23 套件；node-pty 多候选 + 管道显式降级；真机 GUI 冒烟待用户桌面会话）*
