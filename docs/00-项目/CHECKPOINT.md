...
| **验证套件** | 935/935 PASS | `npm run verify`（plugins 88 / orchestration 50 / trace-upload 37 / agent-runtime 40 / agent-loop 14 / model-loop 45 / dsh-runtime 31 / ipc-guard 4 / credentials 34 / data-dir 47 / data-port 10 / session-fork 29 / memory-promotion 22 / memory-summarize 16 / connector-registry 30 / **connector-discover 10** / plugin-market 15 / usage-registry 11 / session-events 16 / ts-loader 13 / browser-tools 44 / terminal-pty 29 / file-panel 20 / file-edit 20 / arch-guard 15 / mcp-client 10 / **skill-pack 10** / e2e 225）—— 28 套件；另设不进链的真机冒烟 `pnpm run smoke:browser`（11/11，需真 GPU/渲染进程） |
...
> **第二十八批（Skill 发布到本地 + 连接器自动发现，2026-09-06）**：用户裁决先补这两个功能增强。① **Skill 发布到本地**——反推本机真实 `.skill`（promo-video.skill 等）：ZIP 且**根目录 = `<slug>/SKILL.md`**（不是 SKILL.md 在 zip 根，生态都这样），可含 references/scripts/；SKILL.md 顶 YAML frontmatter(name/description)。应用内 .skill 从不被解压执行（纯分发产物）。新增 `skill-pack.ts`（**纯逻辑、零依赖 STORE zip + CRC32**，不引 adm-zip/jszip 保零依赖纪律；输出用**系统 python zipfile 跨语言读回**验证——手写 zip 最大风险是「自己写自己读都对、别人读不了」）；`guanji.publishLocalSkill`（slug/description/body → skills/<slug>.skill，临时文件+rename 防半截）；IPC `orchdesk:skill-publish-local` + preload + UI「+ 发布技能到本地」表单（slug/描述/正文 → 打包 → 进已安装清单）。② **连接器自动发现（本地 CLI 登录态）**——新增 `connector-discover.ts`（纯逻辑）：parseGitCredentials（解析 ~/.git-credentials）→ github.com 明文 token 优先命中；gh hosts.yml 仅作「已登录」信号（token 混淆不可读，诚实返 usable=false + 提示 `gh auth token`）；其它连接器本机无标准 CLI 登录态 → 诚实返「无」，不硬凑。**诚实性**：自动发现 ≠ 自动写入——只回填表单，用户点「保存并测试」才落盘（尊重授权）；UI 只给有真实源的 github 显示「自动发现」按钮，不展示假按钮。IPC `orchdesk:connector-discover` + preload + UI。实机验证发现本机 ~/.git-credentials 有两条 github 凭据，parser 取第一条 = 过期的 x-access-token（git push 遮蔽坑的同一来源），安全网靠「保存即探测」让真实 probe 401 暴露。验证：新增 `skill-pack-verify.cjs` 10 项（CRC32 向量/slug 白名单/frontmatter/跨语言 zipfile 读回）+ `connector-discover-verify.cjs` 10 项（解析/discover 各源/诚实降级）；e2e 217→225（发布流程 4 条 + 自动发现 4 条）；全量 verify **907→935**（28 套件）全绿；tsc EXIT=0；arch-guard M4 确认新纯逻辑模块零 electron；知识库审计 0 issues。
...
*最后更新：2026-09-06（第二十八批：Skill 发布到本地 + 连接器自动发现｜verify 935 项全绿；第二十七批：UI/UX 前端收敛（ui-ux-pro-max）｜verify 907 项全绿；第二十六批：MCP 真接入（零依赖 stdio 客户端）｜verify 896 项全绿；第二十五批：右栏「能力」TAB 改名 + 技能改磁盘真实扫描｜verify 884 项全绿）*
