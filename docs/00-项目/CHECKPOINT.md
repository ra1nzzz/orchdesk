...
| **验证套件** | 741/741 PASS | `npm run verify`（plugins 88 / orchestration 45 / trace-upload 36 / agent-runtime 38 / agent-loop 14 / model-loop 40 / dsh-runtime 31 / credentials 34 / data-dir 47 / data-port 10 / session-fork 29 / memory-promotion 22 / memory-summarize 16 / connector-registry 30 / plugin-market 15 / usage-registry 11 / session-events 16 / ts-loader 13 / **browser-tools 39** / arch-guard 15 / e2e 152）—— 21 套件 |
...
*最后更新：2026-09-01（第十三批：浏览器工具（ADR-0011，Electron 自带 CDP，8 个工具）｜verify 702→741，21 套件；另设不进链的真机冒烟 `pnpm run smoke:browser`）*
