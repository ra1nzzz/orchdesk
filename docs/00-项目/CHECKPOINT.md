...
> **第二十九批（回滚：移除 Skill 发布到本地，2026-09-06）**：用户裁决「暂时把这个功能移除，保留从观雅集下载 SKILL 的功能」——上一批加的「Skill 发布到本地」（表单新建 .skill 打包）方向不合意，回滚到原始状态。移除：UI「+ 发布技能到本地」按钮/表单/两个事件 handler、IPC `orchdesk:skill-publish-local`、preload `publishLocalSkill` 桥、`guanji.publishLocalSkill` 方法 + `LocalPublishResult` 类型、`skill-pack.ts`（零依赖 STORE zip 打包器）及 `skill-pack-verify.cjs`（10 项，含 python zipfile 跨语言校验）、arch-guard PURE_MODULES 登记、verify 链、e2e mock+4 条发布断言（225→221）。**保留**：从观雅集下载 skill（`guanji.listSkills`+`installSkill`，既有「安装」按钮）、已安装技能清单的启用/停用/卸载、`guanji.publishSkill`（发到观雅集，原本就有）。技巧沉淀保留：.skill 真实格式（ZIP 根 = `<slug>/SKILL.md`）写入长期记忆（MEMORY.md），后续若要重新支持本地打包可直接复用。验证：tsc EXIT=0；e2e 225→221；全量 verify **935→921**（28→27 套件，去掉 skill-pack 10 项）全绿。

> **第二十八批（连接器自动发现（本地 CLI 登录态），2026-09-06）**：用户裁决先补连接器自动发现。新增 `connector-discover.ts`（纯逻辑）：parseGitCredentials（解析 ~/.git-credentials）→ github.com 明文 token 优先命中；gh hosts.yml 仅作「已登录」信号（token 混淆不可读，诚实返 usable=false + 提示 `gh auth token`）；其它连接器本机无标准 CLI 登录态 → 诚实返「无」，不硬凑。**诚实性**：自动发现 ≠ 自动写入——只回填表单，用户点「保存并测试」才落盘（尊重授权）；UI 只给有真实源的 github 显示「自动发现」按钮，不展示假按钮。IPC `orchdesk:connector-discover` + preload + UI。实机验证发现本机 ~/.git-credentials 有两条 github 凭据，parser 取第一条 = 过期的 x-access-token（git push 遮蔽坑的同一来源），安全网靠「保存即探测」让真实 probe 401 暴露。验证：`connector-discover-verify.cjs` 10 项（解析/discover 各源/诚实降级）+ e2e 4 条自动发现断言；全量 verify 到 921 项（27 套件）全绿（注：本批原本还含 Skill 发布到本地，已在第二十九批回滚）。
 297→
...
*最后更新：2026-09-06（第二十九批：回滚移除 Skill 发布到本地，保留观雅集下载｜verify 921 项全绿；第二十八批：连接器自动发现（本地 CLI 登录态）｜verify 921 项全绿；第二十七批：UI/UX 前端收敛（ui-ux-pro-max）｜verify 907 项全绿；第二十六批：MCP 真接入（零依赖 stdio 客户端）｜verify 896 项全绿）*
