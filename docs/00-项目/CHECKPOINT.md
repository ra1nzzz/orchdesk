...
| **验证套件** | 745/745 PASS | `npm run verify`（plugins 88 / orchestration 45 / trace-upload 36 / agent-runtime 38 / agent-loop 14 / model-loop 40 / dsh-runtime 31 / credentials 34 / data-dir 47 / data-port 10 / session-fork 29 / memory-promotion 22 / memory-summarize 16 / connector-registry 30 / plugin-market 15 / usage-registry 11 / session-events 16 / ts-loader 13 / **browser-tools 43** / arch-guard 15 / e2e 152）—— 21 套件；另设不进链的真机冒烟 `pnpm run smoke:browser`（11/11，需真 GPU/渲染进程） |
...
*最后更新：2026-09-01（第十四批：浏览器工具真机冒烟 11/11 + CDP 挂起类降级三条（ADR-0011）｜verify 741→745，21 套件；真机冒烟 `pnpm run smoke:browser` 已在本机跑通）*
