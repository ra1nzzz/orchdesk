...
export function apply(ctx: Context, _config: MultiConfig): void {
  ...
  function getDelegationTree(rootId?: string): DelegationNode[] {
    ...
    return rootId ? all.filter((n) => n.id === rootId || n.parentId === rootId) : all;
  }
  ...
  async function composeTeam(
    ...
    try {
      ...
      ceo.status = 'done';
      for (const n of tree.values()) if (n.id !== rootId) n.status = 'done';
  ...
  ctx.effect(() => {
    ...
    anyCtx.provide?.('orchestration', api, true);
    return () => {
      tree.clear();
    };
